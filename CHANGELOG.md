...
### Changed
- Refactored `ConnectionManager` and API client architecture — unified `ApiClient` with Bearer token auth and `GatewayChatClient` for SSE streaming, replacing the previous `DashboardClient` cookie-based approach.
